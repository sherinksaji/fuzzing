...
async def write_target(target, attribute, bytes, PermissionsAct):
    ...
    try:
        ...
        if (
            split[0][0] == "S"
            and PermissionsAct[attribute.handle][1].find("WRITE") == -1
        ):
            print(
            ...
            print(
                f"BUG FOUND. This characeristic of handle 0x{attribute.handle:04X} does not have WRITE permission"
            )
        return True
        ...
        if (
            ...
            print(
                f"BUG FOUND. This characeristic of handle 0x{attribute.handle:04X} has WRITE permission, but is not being WRITTEN to"
            )
    except TimeoutError:
...
async def read_target(target, attribute, PermissionsAct):
    ...
    try:
        ...
        if (
            split[0][0] == "S"
            and PermissionsAct[attribute.handle][1].find("READ") == -1
        ):
            print(
            ...
            print(
                f"BUG FOUND. This characeristic of handle 0x{attribute.handle:04X} does not have READ permission"
            )
        ...
        if (
            ...
            print(
                f"BUG FOUND. This characeristic of handle 0x{attribute.handle:04X} has READ permission, but is not being READ"
            )
